...
```json
{
  ...
  "tool_name": "Edit",
  "tool_input": {
    "file_path": "/root/git/your-project/backend/src/services/GameService.java",
    "old_string": "...",
    "new_string": "..."
  }
}
...
File: backend/src/services/GameService.java
...
1. **First edit** of file with matching pattern:
   - Hook blocks with exit code 2
   - Updates session state: adds skill to skills_used
   - Claude sees message, uses skill

2. **Second edit** (same session):
   - Hook checks session state
   - Finds skill in skills_used
...
- Example: `backend/src/services/**` better than `backend/**`
